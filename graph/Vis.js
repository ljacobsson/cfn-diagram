"use strict";
import { createPseudoResources, pathToDescriptor, findAllValues } from "../resources/JsonUtil.js";
import filterConfig from "../resources/FilterConfig.js";
import tempDirectory from "temp-dir";
import { templates } from "../shared/templateCache.js";
import { existsSync, mkdirSync, readFile, writeFile, copyFileSync, writeFileSync } from "fs";
import { join } from "path";
import open from "open";
import ColorHash from "color-hash";
import { yamlParse, yamlDump } from "yaml-cfn";
import { getSVG } from "aws-icons-directory";
...
function addEdges(from, to, dependencyNode, fromNode) {
  if (from && to) {
    if (dependencyNode.path.indexOf("Properties.Events") > 0) {
      ...
      const descriptor = pathToDescriptor(
        dependencyNode.path,
        filterConfig
      );
      if (
...
export async function renderTemplate(
  ...
  `;
  const uiPath = filePath || join(tempDirectory, "cfn-diagram");
  if (!existsSync(uiPath)) {
    mkdirSync(uiPath);
  }
  if (standaloneIndex) {
    readFile(join(__dirname, "ui", "index_standalone.template"), 'utf8', function (err, data) {
      if (err) {
        return console.log(err);
      }
      var result = data.replace(/%DATA_JS%/g, fileContent);

      writeFile(join(uiPath, "index.html"), result, 'utf8', function (err) {
  ...
  copyFileSync(
    ...
    join(uiPath, "index.html")
  );
  copyFileSync(
    join(__dirname, "ui", "icons.js"),
    join(uiPath, "icons.js")
  );
  writeFileSync(join(uiPath, "data.js"), fileContent);
  }
  if (!ciMode) {
    open(join(uiPath, "index.html"));
  }
}
